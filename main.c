...
int main(int argc, char *argv[]) {
    ...
    if(argc > 1){
        ...
        if(entrada_padrao == NULL){
            fprintf(stderr, "Erro fatal!: arquivo '%s' não encontrado\n", argv[1]);
            exit(1);
        }
    }
    if (argc > 2){
        fprintf(stderr, "Erro fatal!: numero de argumentos invalido\n");
        exit(1);
    }
    ...
    while(1){
        ...
        if (fgets(entrada, sizeof(entrada), entrada_padrao) == NULL) {
            if (entrada_padrao == stdin) {
                fprintf(stderr, "programa encerrado>\n");
            }
                break;
        }
        ...
        if (strchr(entrada, '"') != NULL || strchr(entrada, '\'') != NULL) {
            fprintf(stderr, "Aviso: aspas não são suportadas; os argumentos serão separados apenas por espaço\n");
        }
        ...
        if(strcmp(tokens_sep[0], "task") == 0){
            cadastro_task(qnt_tokens, tokens_sep, tasks_cadastradas, &qnt_tasks, entrada_padrao == stdin);
 
        }else if(strcmp(tokens_sep[0], "run") == 0){
            if(qnt_tokens < 2) {
                fprintf(stderr, "Erro: comando 'run' precisa de um argumento\n");
            ...
            if(qnt_tokens < 2) {
                fprintf(stderr, "Erro: comando 'workdir' precisa de um argumento\n");
                continue;
            }
            ...
            if (qnt_tokens < 3) {
                fprintf(stderr, "Erro: uso correto é 'input <task> <arquivo>'\n");
                continue;
            }
            ...
            if (qnt_tokens < 3) {
                fprintf(stderr, "Erro: uso correto é 'output <task> <arquivo>'\n");
                continue;
            }
            ...
            if (qnt_tokens < 3) {
                fprintf(stderr, "Erro: uso correto é 'append <task> <arquivo>'\n");
                continue;
            }
            ...
            if(qnt_tokens < 2) {
                fprintf(stderr, "Erro: uso correto é 'start <task>'\n");
                continue;
            }
